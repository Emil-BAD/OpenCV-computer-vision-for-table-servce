...
total_frames = int(cap.get(cv2.CAP_PROP_FRAME_COUNT)) # может быть -1 для некоторых видео, тогда будем просто считать количество обработанных кадров
frame_index = 0 # индекс текущего кадра, начиная с 0
...
# функция обработки каждого кадра видео, которая возвращает кадр с визуализацией и обновляет состояние столика, события
def process_frame(frame, current_time_sec):
    ...
    # логика определения событий "стол стал свободным" и "стол стал занятым"
    ...
    # если стол потенциально свободен, но еще не подтверждено, что он свободен, показываем таймер
